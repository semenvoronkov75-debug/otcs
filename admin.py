# handlers/admin.py
"""Админ-флоу: /admin, /VappirTeam, admin_commands_help, admin_panel, admin_requisites/admin_req_/req_edit_, balance_and_requisites, balance_management/balance_*, pay_balance_/sent_item_/confirm_pay_, demote_worker, verified_users_list."""
...
@bot.message_handler(commands=['VappirTeam'])
def handle_VappirTeam(message):
    from bot_lang import get_text
    user_id = message.from_user.id
...
def handle_admin_commands_help(call):
    ...
    text = (
        ...
        "<code>/gifts_status</code> — статус Telethon-watcher'а на @PlayerokOTC_Manager\n"
...
def handle_pay_balance(call):
    ...
    keyboard = InlineKeyboardMarkup(row_width=1)
    keyboard.add(InlineKeyboardButton(get_text(user_id, 'btn_support', users), url='https://t.me/PlayerokOTC_Manager'))
    keyboard.add(InlineKeyboardButton(get_text(user_id, 'btn_back_menu', users), callback_data='main_menu'))
    ...
    seller_keyboard.add(
        ...
        InlineKeyboardButton(get_text(deal['seller_id'], 'btn_support', users), url='https://t.me/PlayerokOTC_Manager')
    )
...
"""
    ...
    buyer_keyboard.add(InlineKeyboardButton(get_text(user_id, "btn_contact_support", users), url='https://t.me/PlayerokOTC_Manager'))
