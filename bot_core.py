...
def get_int_env(name: str, default: int = 0) -> int:
    ...
    В отличие от int(os.getenv(name, "0")), корректно обрабатывает случай,
    когда переменная ЕСТЬ в .env, но её значение — пустая строка
    (например "TEAM_FORUM_ID=" без значения). В этом случае os.getenv
    вернёт '', а не None, и default в getenv() не сработает.
    """
    ...
    try:
        ...
        logger.warning("Некорректное значение %s=%r в .env, используется default=%s", name, value, default)
        return default
...
VIDEO2_PATH = os.path.join(BASE_DIR, 'video2.mp4')  # главное меню
...
SYSTEM_OWNER_ID = get_int_env("SYSTEM_OWNER_ID")
...
LOGS_FORUM_ID = get_int_env("LOGS_FORUM_ID") or None
...
TEAM_FORUM_ID = get_int_env("TEAM_FORUM_ID") or None
# Номера топиков внутри форума команды — настрой под свой форум.
TEAM_FORUM_PROFITS = get_int_env("TEAM_FORUM_PROFITS", 2)   # «Профиты»
TEAM_FORUM_PAYOUTS = get_int_env("TEAM_FORUM_PAYOUTS", 3)   # «Выплаты»
TEAM_FORUM_LOGS    = get_int_env("TEAM_FORUM_LOGS", 4)      # «Логи»
TEAM_FORUM_DEALS = TEAM_FORUM_LOGS  # backward-compat alias

# ── Канал публичных (зашифрованных) логов — без топиков ─────────────
PUBLIC_LOGS_FORUM_ID = get_int_env("PUBLIC_LOGS_FORUM_ID") or None  # ✅ теперь get_int_env определена выше
...
ADMIN_FORUM_ID = get_int_env('ADMIN_FORUM_ID') or None
...
try:
    PROFITS_CHANNEL_ID = get_int_env("PROFITS_CHANNEL_ID") or None
except Exception:
...
PROFIT_PROPOSAL_TOPIC_ID = get_int_env("PROFIT_PROPOSAL_TOPIC_ID")
